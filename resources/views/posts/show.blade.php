    ...
    <div class="container mx-auto md:flex">
            ...
            @auth
                @if($post->user_id === auth()->user()->id)
                    <form method="POST" action="{{route('posts.destroy', $post)}}">
                        @method('DELETE')
                        @csrf
                        <input type="submit" value="Eliminar Publicación" class="bg-red-500 p-2 rounded text-white mt-4 cursor-pointer">
                    </form>
                @endif
            @endauth
        ...
        <div class="md:w-1/2 P-5">
            <div class="shadow bg-white p-5 mb-5">
                @auth
                    <p class="font-bold text-center mb-4 text-xl">Agrega un nuevo comentario</p>
                    ...
                    <form action="{{route('comentarios.store', ['user' => $user, 'post' => $post])}}" method="POST">
                        ...
                        <div class="mb-5">
                            ...
                            <textarea id="comentario" name="comentario" class="border p-3 w-full rounded-lg @error('comentario') border-red-500 @enderror" placeholder="Agrega un comentario"></textarea>
                            @error('comentario')
